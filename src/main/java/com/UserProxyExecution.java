...
public class UserProxyExecution {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            fileStream = new FileOutputStream(new File("execute_user_proxy.sql"));
            bufferStream = new BufferedOutputStream(fileStream);
            UserMigration03.executeUpdateUserProxy(resultSet, statement, bufferStream);
        } catch (Exception e) {
            logger.error(e.getMessage());
        } finally {
            try {
                logger.info("UserProxyExecution done! please execute execute_user_proxy.sql!!!");
                logger.info("UserProxyExecution done! please execute execute_user_proxy.sql!!!");
                logger.info("UserProxyExecution done! please execute execute_user_proxy.sql!!!");
